...
static int PivVerifyPin(Iso7816CommandChannel channel, uint8_t pin[], size_t pin_len, bool localPin, bool verbose, uint16_t *sw) {
    uint8_t pin_buf[8] = {0};
    memset(pin_buf, 0xff, sizeof(pin_buf));
    if (pin_len == 0 || pin_len > sizeof(pin_buf)) {
        return PM3_EINVARG;
    }
    memcpy(pin_buf, pin, pin_len);

    uint8_t buf[APDU_RES_LEN] = {0};
    size_t len = 0;
    uint8_t pin_type_p2 = 0x00;  // global PIN
    if (localPin == true) {
        pin_type_p2 = 0x80;
    }
    int res = Iso7816ExchangeEx(channel, false, true, (sAPDU_t) {0x00, 0x20, 0x00, pin_type_p2, sizeof(pin_buf), pin_buf}, false, 0, buf, APDU_RES_LEN, &len, sw);
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(FAILED, "Sending APDU failed with code %d", res);
        *sw = 0;
        return res;
    ...
    if (*sw == APDU_RES_SUCCESS) {
        if (verbose == true) {
            PrintAndLogEx(SUCCESS, "PIN is valid");
        }
        return PM3_SUCCESS;
    }
    if ((*sw & 0xfff0) == 0x63C0) {
        if (verbose == true) {
            PrintAndLogEx(WARNING, "PIN is INVALID. %u tries left before locking the card.", res & 0x0f);
        }
        return PM3_EFAILED;
    }
    if (*sw == APDU_RES_PIN_BLOCKED) {
        if (verbose == true) {
            PrintAndLogEx(FAILED, "PIN is blocked!");
        }
        return PM3_EFAILED;
    }
    if (verbose == true) {
...
static int CmdPIVPinVerify(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "piv verifypin",
                  "Verifies the PIN",
                  "piv verify -sk 1234   -> select card, select applet, verify pin and keep field on for next command\n");

    void *argtable[] = {
        arg_param_begin,
        arg_lit0(NULL,  "local",   "Authenticate using local PIN instead of global one (OpenFIPS201)"),
        arg_lit0("sS",  "select",  "Activate field and select applet"),
        arg_lit0("kK",  "keep",    "Keep field for next command"),
        arg_lit0("aA",  "apdu",    "Show APDU requests and responses"),
        arg_lit0("tT",  "tlv",     "TLV decode results"),
        arg_lit0("wW",  "wired",   "Send data via contact (iso7816) interface. (def: Contactless interface)"),
        arg_str0(NULL,  "aid", "<hex>", "Applet ID to select. By default A0000003080000100 will be used"),
        arg_str1(NULL, NULL, "<hex PIN>", "PIN to verify"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);

    bool localPin = arg_get_lit(ctx, 1);
    bool activateField = arg_get_lit(ctx, 2);
    bool leaveSignalON = arg_get_lit(ctx, 3);
    ...
    uint8_t applet_id[APDU_AID_LEN] = {0};
    int aid_len = 0;
    CLIGetHexWithReturn(ctx, 7, applet_id, &aid_len);
    if (aid_len == 0) {
        memcpy(applet_id, PIV_APPLET, sizeof(PIV_APPLET));
        aid_len = sizeof(PIV_APPLET);
    }
    ...
    uint8_t pin[8] = {0};
    int pin_len = 0;
    CLIGetHexWithReturn(ctx, 8, pin, &pin_len);

    CLIParserFree(ctx);

    if (pin_len > 8) {
        PrintAndLogEx(WARNING, "Invalid PIN. Size should be at most 8 bytes. Got %d", pin_len);
        return PM3_EINVARG;
    }
    ...
    uint16_t sw = 0;
    res = PivVerifyPin(channel, pin, pin_len, localPin, true, &sw);
    if (leaveSignalON == false) {
        DropFieldEx(channel);
    }
    return res;
...
static command_t CommandTable[] =  {
    ...
    {"verifypin",   CmdPIVPinVerify,                IfPm3Iso14443,   "Verifies the PIN"},
    {"auth",        CmdPIVAuthenticateSign,         IfPm3Iso14443,   "Authenticate with the card"},
